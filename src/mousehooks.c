...
void	scroll_through(double xdelta, double ydelta, void *param)
{
	...
	if (sim->pos[0] < 0 || (uint32_t)sim->pos[0] >= sim->canvas->width
		||sim->pos[1] < 0 || (uint32_t)sim->pos[1] >= sim->canvas->height)
		return ;
	tempscale = 0.9 + 0.2 * (ydelta > 0);
	c = map_to_complex(sim->fract, sim->pos[0], sim->pos[1]);
	sim->fract->plane->x_max = c.real
		+ (sim->fract->plane->x_max - c.real) * tempscale;
	sim->fract->plane->x_min = c.real
		+ (sim->fract->plane->x_min - c.real) * tempscale;
	sim->fract->plane->y_max = c.imag
		+ (sim->fract->plane->y_max - c.imag) * tempscale;
	sim->fract->plane->y_min = c.imag
		+ (sim->fract->plane->y_min - c.imag) * tempscale;
	sim->scale *= tempscale;
	need_redraw(sim);
}
...
void	translate_viewport(t_sim *sim)
{
	...
	pos[0] = map_to_complex(sim->fract,
		sim->mos.start[0], sim->mos.start[1]);
	pos[1] = map_to_complex(sim->fract,
		sim->mos.end[0], sim->mos.end[1]);
	pos[0] = (t_complex){pos[0].real - pos[1].real,
		pos[0].imag - pos[1].imag};
	sim->fract->plane->x_max -= pos[0].real;
	sim->fract->plane->y_max -= pos[0].imag;
	sim->fract->plane->x_min -= pos[0].real;
	sim->fract->plane->y_min -= pos[0].imag;
